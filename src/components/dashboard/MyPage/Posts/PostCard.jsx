...
function PostCard({ data }) {
  ...
  return (
    <div className="lg:flex-row flex flex-col gap-4 lg:p-6 p-4 bg-gray-50 rounded-xl h-full">
      <div className="lg:w-3/12 relative w-full overflow-hidden h-[300px] lg:h-full rounded-xl">
        {/* Overlay to block visibility */}
        <div className="h-full absolute top-0 left-0 w-full bg-gradient-to-t from-[rgba(0,0,0,0.5)] to-[rgba(255,255,255,0.5)] backdrop-blur-sm rounded-xxl z-10 flex items-center justify-center ">
          <svg
            onClick={() => {
        ...
        {imageUrl ? (
          ...
          <audio className="w-full h-full z-0" controls>
            <source
              src={audioUrl}
          ...
          <video className="w-full h-full z-0" controls>
            <source
              src={videoUrl}
